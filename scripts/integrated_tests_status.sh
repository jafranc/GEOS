...
tests_modified=$(curl -H "Accept: application/vnd.github+json" \
  https://api.github.com/repos/GEOSX/GEOSX/pulls/$TRAVIS_PULL_REQUEST/files?per_page=100 | \
...
commit_msg=$(curl -H "Accept: application/vnd.github+json" \
  https://api.github.com/repos/GEOSX/GEOSX/branches/next | \
...
commit_state=$(curl -H "Accept: application/vnd.github+json" \
  https://api.github.com/repos/GEOSX/GEOSX/commits/$commit_sha/status | \
  jq ".state")
